...
case "needstate":
  ...
  function ensureUi() {
    ...
    makeDraggable(ui.root.querySelector(".wt-drag-handle"), ui.panel);
    makeDraggable(ui.quickTitle, ui.quickPanel);
    makeDraggable(ui.quickBar, ui.quickPanel);
  }

  function makeDraggable(dragHandle, target) {
...
// ---------- boot ----------
  ...
  (async () => {
    ...
    const { wt_party: party, wt_voice_vol: savedVol, wt_react_freq: savedFreq, wt_react_recent: savedRecent } = await chrome.storage.local.get(["wt_party", "wt_voice_vol", "wt_react_freq", "wt_react_recent"]);

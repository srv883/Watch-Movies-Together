(() => {
  ...
  async function getMicStream() {
    ...
    const stream = await navigator.mediaDevices.getUserMedia({
      audio: { echoCancellation: true, noiseSuppression: !!S.nc, autoGainControl: true }
    });
    S.micStream = stream;
    return stream;
  }
  ...
  function hotSwapMicTrack(stream) {
    try {
      ...
      const nt = stream && stream.getAudioTracks()[0];
      for (const sender of pc.getSenders()) {
        if (sender.track && sender.track.kind === "audio" && typeof sender.replaceTrack === "function") {
          try { sender.replaceTrack(nt || null); } catch (_) {}
        }
      }
    } catch (_) {}
  }
...
function toast(text) {
  ...
  (async () => {
    ...
    const { wt_party: party, wt_voice_vol: savedVol, wt_react_freq: savedFreq, wt_react_recent: savedRecent, wt_glass: savedGlass, wt_pos_root: posRoot, wt_pos_quick: posQuick, wt_nc: savedNc } = await chrome.storage.local.get(["wt_party", "wt_voice_vol", "wt_react_freq", "wt_react_recent", "wt_glass", "wt_pos_root", "wt_pos_quick", "wt_nc"]);

(() => {
  ...
  function startVoice() {
    if (S.micStream || !S.peer || !isOpen()) return;
    navigator.mediaDevices
      .getUserMedia({ audio: { echoCancellation: true, noiseSuppression: true } })
      .then((st) => {
        if (S.stopping || !isOpen()) {
          st.getTracks().forEach((t) => t.stop());
          return;
        }
        S.micStream = st;
        if (S.role === "guest") {
          try { S.call = S.peer.call(PEER_PREFIX + S.roomId, st); } catch (_) {}
          wireCall(S.call);
        }
      })
      .catch(() => sysMsg("Mic blocked \u2013 allow microphone in the address bar for voice"));
  }

  function onIncomingCall(c) {
    if (S.role !== "host" || !isOpen()) {
      try { c.close(); } catch (_) {}
      return;
    }
    const answer = (st) => {
      try { c.answer(st); } catch (_) {}
      wireCall(c);
    };
    if (S.micStream) {
      answer(S.micStream);
    ...
    navigator.mediaDevices
      ...
      .catch(() => {
        sysMsg("Mic blocked \u2013 allow microphone in the address bar for voice");
        answer();
      });
  }
  ...
  function wireCall(call) {
    ...
    call.on("stream", (remote) => {
      ...
      if (!S.voiceAnnounced) {
        ...
        sysMsg("Voice connected");
      }
    });
  }

  function ensureRemoteAudio() {
    if (S.remoteAudio) return S.remoteAudio;
    S.remoteAudio = document.createElement("audio");
    S.remoteAudio.autoplay = true;
    S.remoteAudio.style.display = "none";
    document.documentElement.appendChild(S.remoteAudio);
    return S.remoteAudio;
  }

  function toggleMute() {
    if (!S.micStream) {
      sysMsg("Voice not active yet");
      return;
    }
    S.muted = !S.muted;
    for (const t of S.micStream.getAudioTracks()) t.enabled = !S.muted;
    updateMicBtn();
  }

  function updateMicBtn() {
    if (!ui.micBtn) return;
    ui.micBtn.textContent = S.muted ? "Unmute" : "Mute";
    ui.micBtn.classList.toggle("wt-muted", S.muted);
  }

  function stopVoice() {
    if (S.call) {
      try { S.call.close(); } catch (_) {}
      S.call = null;
    }
    if (S.micStream) {
    ...
    if (S.remoteAudio) {
      try { S.remoteAudio.remove(); } catch (_) {}
      S.remoteAudio = null;
    }
    S.muted = false;
  ...
  function ensureUi() {
    ...
    ui.root.innerHTML =
      ...
      '<button id="wt-mic" title="Toggle microphone">Mute</button>' +

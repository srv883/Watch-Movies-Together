(() => {
      ...
      if (!swapped && nt && typeof pc.getTransceivers === "function") {
        for (const tr of pc.getTransceivers()) {
          ...
          if (kind === "audio" && tr.sender && typeof tr.sender.replaceTrack === "function") {
            try { tr.sender.replaceTrack(nt); break; } catch (_) {}
          }
        }
      }
    } catch (_) {}
  ...
  // additionally hard-gate the mic track: RMS below threshold for ~0.7s sends
  // digital silence (track.enabled=false), instant reopen when you speak.
  function stopGate() {
  ...
  function startGate() {
    ...
    try {
      ...
      S.gateTimer = setInterval(() => {
        try {
          if (!S.micStream || !S.micStream.active) { stopGate(); return; }
          ...
          if (rms < 0.012) {
            if (++quietTicks >= 6 && !S.gateMuted) {
              S.gateMuted = true;
              for (const t of tracks) { try { t.enabled = false; } catch (_) {} }
            }
          } else {
            quietTicks = 0;
            if (S.gateMuted) {
              S.gateMuted = false;
              for (const t of tracks) { try { t.enabled = true; } catch (_) {} }
            }
          }
        } catch (_) {}
      }, 120);
  ...
  function attachVoiceCall(call) {
    ...
    try {
      if (S.micEnabled && S.micStream && S.micStream.active) hotSwapMicTrack(S.micStream);
  ...
  async function ensureMicIfWanted() {
    ...
    try {
      ...
      setMicUi(true);
      if (S.voiceConn) hotSwapMicTrack(S.micStream);
      if (!!S.nc) startGate();
    } catch (_) {}
  }

/* ============================================================
   V10 behaviour test harness (port of test-V9 + v10 additions)
   Runs the REAL V10-dropfix-nc/content.js twice (host+guest)
   against stubbed DOM / PeerJS / chrome APIs and verifies the
   full feature set end-to-end, plus:
     - built-in noise cancellation toggle (live constraint apply)
     - connection-drop resilience (calm status, no restart storms)
   Run:  node tests/test-V10.js
   ============================================================ */
"use strict";
...
const SRC_PATH = "C:/Users/Sourav/ML Coding/watch-together-versions/V10-dropfix-nc/content.js";
const DIR10 = "C:/Users/Sourav/ML Coding/watch-together-versions/V10-dropfix-nc";
...
/* ============================================================ */
async function main() {
  const env = { clock: new Clock(), errs: [], gum: 0, cutoff: false, FakeStream: FakeStreamForGum, _peerSeq: 0, _rtc: [], micTracks: [], lastGum: null };
  ...
  const MF = JSON.parse(fs.readFileSync(DIR10 + "/manifest.json", "utf8"));
  ok(MF.content_scripts[0].matches.indexOf("<all_urls>") !== -1, "content script runs on ALL sites (<all_urls>)");
  const POP = fs.readFileSync(DIR10 + "/popup.js", "utf8");
...
console.log("\n--- 12. error sweep ---");
  ...
  main().catch(e => { const s = "HARNESS CRASH: " + (e && e.stack || e); try { fs.writeFileSync("C:/Users/Sourav/ML Coding/watch-together-versions/tests/crash10.txt", s); } catch (_) {} process.exitCode = 2; });

/* ============================================================
   V11 behaviour test harness (port of test-V10 + v11 additions)
   Runs the REAL V13-lowlatency/content.js twice (host+guest)
   against stubbed DOM / PeerJS / chrome APIs. New for v11:
     - mute keeps the voice line open (no re-dial, no deafening)
     - silent-track answer slots + hotSwap assertions
     - software noise gate (env.rms drives the analyser)
     - watch history accrual / persistence / UI
   Run:  node tests/test-V11.js
   ============================================================ */
"use strict";
...
const SRC_PATH = "C:/Users/Sourav/ML Coding/watch-together-versions/V13-lowlatency/content.js";
const DIR12 = "C:/Users/Sourav/ML Coding/watch-together-versions/V13-lowlatency";
...
console.log("\n--- 12. error sweep ---");
  ...
  ok(env.lastGum && env.lastGum.audio && env.lastGum.audio.noiseSuppression === true,
    "mic acquired with noiseSuppression=true");
  ncBtn.click();
  await step(clock, 60);
  ...
  ok(liveTrack.lastApply && liveTrack.lastApply.noiseSuppression === false,
    "live mic stream constraint flipped to false without re-dial (" + JSON.stringify(liveTrack.lastApply) + ")");
  ncBtn.click();
  await step(clock, 60);
  ...
  ok(liveTrack.lastApply && liveTrack.lastApply.noiseSuppression === true,
    "live mic stream constraint flipped back to true");
  ...
  // constraints now request hi-res mono + AGC + EC
  ok(env.lastGum && env.lastGum.audio && env.lastGum.audio.sampleRate === 48000 &&
    ...
    "mic constraints: 48kHz mono + EC + AGC (" + JSON.stringify(env.lastGum && env.lastGum.audio) + ")");
  // opus bitrate boost installed in source
  ok(SRC.includes("maxaveragebitrate=96000") && SRC.includes("usedtx=0"),
    "Opus SDP boost present (96kbps stereo, no DTX)");
  ...
  ok(gateTrack.enabled === false, "gate DISABLES mic in sustained silence");
  env.rms = 0.25;    // someone speaks
  await step(clock, 280); // v13: two voiced ticks to reopen
  ok(gateTrack.enabled === true, "gate re-opens on sustained voice (~0.24s)");
  env.rms = 0.2;
  ...
  await step(clock, 400);
  for (let i = 0; i < 40; i++) await step(clock, 120); // warmup 1.7s + hold 2.4s
  ok(gpTrack.enabled === false, "genuine sustained silence still gates the mic");
  ok(GP12.__ctx.__wtVoiceDbg().gateMuted === true, "gate state reported as muted");
  GP12.q("#wt-nc-btn").click(); // NC off -> stopGate must restore the track
  await step(clock, 300);
  ok(gpTrack.enabled === true, "turning NC off UN-GATES a gated mic track");
  ...
  ok(gTrack13.enabled === false, "v13 gate mutes only after sustained TRUE idle");
  env.rms = 0.3;
  await step(clock, 130);
  ok(gTrack13.enabled === false, "single loud blip does NOT flip the stream (no flap)");
  await step(clock, 140);
  ok(gTrack13.enabled === true, "sustained voice reopens within ~0.24s");
  env.rms = 0.2;
  ...
  main().catch(e => { const s = "HARNESS CRASH: " + (e && e.stack || e); try { fs.writeFileSync("C:/Users/Sourav/ML Coding/watch-together-versions/tests/crash13.txt", s); } catch (_) {} process.exitCode = 2; });

/* ============================================================
   ...
   Runs the REAL V12-remote-audio/content.js twice (host+guest)
...
const SRC_PATH = "C:/Users/Sourav/ML Coding/watch-together-versions/V12-remote-audio/content.js";
const DIR12 = "C:/Users/Sourav/ML Coding/watch-together-versions/V12-remote-audio";
...
function makePeerClass(env) {
  ...
  class Call {
    constructor(outStream) {
      ...
      this.pcCaller = { getSenders: () => self.sendersOut, getTransceivers: () => [] };
      this.pcCallee = { getSenders: () => self.sendersIn, getTransceivers: () => [] };
    }
    emitTo(h, t, ...x) {
      (h[t] || []).slice().forEach(f => {
...
console.log("\n--- 12. error sweep ---");
  ...
  for (let i = 0; i < 26; i++) await step(clock, 120); // > warmup(1.7s) + hold(0.7s)
  ok(gateTrack.enabled === false, "gate DISABLES mic in sustained silence");
  env.rms = 0.25;    // someone speaks
  await step(clock, 150);
  ok(gateTrack.enabled === true, "gate re-opens instantly on voice");
  env.rms = 0.2;
  ...
  main().catch(e => { const s = "HARNESS CRASH: " + (e && e.stack || e); try { fs.writeFileSync("C:/Users/Sourav/ML Coding/watch-together-versions/tests/crash12.txt", s); } catch (_) {} process.exitCode = 2; });
